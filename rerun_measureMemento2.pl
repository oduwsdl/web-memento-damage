...
if($#ARGV < 1)
{
        print "USAGE: perl measureMemento.pl [404 File] [URI-M]\n";
        exit();
}
...
if($urim)
{
	...
	my $pixelCoverage = pctCoverage($the404file);
	#print "Covering $pixelCoverage\n";

	##3)
	my @theMissing = findMissing($the404file);
	...
	my $potentialDamage = findPotentialDamage($the404file, $MMweight, 
			$CSSweight, $IMGweight, $TXTweight, $wordsPerImg);

	#print "Potential DONE\n\n";
	#print "After Weights: $CSSweight, $IMGweight, $TXTweight, $MMweight\n";

	print "\n\nProcessing actual damage in $the404file...\n\n";
	my %actualDamages = findActualDamage($the404file, $MMweight, 
			$CSSweight, $IMGweight, $TXTweight, $wordsPerImg, 
			$potentialDamage);
...
sub findPotentialDamage($)
{
	...
	for(my $i = 0; $i <= $#uris; $i++)
	{
		...
		unless($uris[$i] eq "http://web.archive.org/static/css/styles.css")
		{
			$damages{$u} = findImportance($uris[$i], "$baseFile.png", 
...
sub findActualDamage($)
{
	...
	my @uris = findMissing("$baseFile.404");

	open(FILE, "$toSave.404");
	my @alluris = <FILE>;
	close(FILE);
	
	print "Finding actual damage in $targetFile.404...\n";
...
sub findCssImportance($)
{
	...
	if($isPotential == 0)
	{
		open(OUT, "/home/jbrunelle/public_html/wsdl/damage_revist/dataFiles/CSSdistroIA.csv");
                my @cssLines = <OUT>;
               	close(OUT);
		...
		for(my $cl = 0; $cl < $#cssLines+1; $cl++)
		{
			my @arr = split(/, /, trim($cssLines[$cl]));
			my $toTest = trim($arr[0]);

			if($thePng =~ m/$toTest/ig)
			{
				print "Found the PNG\n\n";
				$low = trim($arr[1]);
				$mid = trim($arr[2]);
