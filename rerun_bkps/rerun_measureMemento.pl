...
sub findCssImportance($)
{
	...
	if($isPotential == 0)
	{
		$cmd = "perl ./memDamageDistro/rerun_whitespace.pl $thePng";
		#print "Running...$cmd\n";
		...
		open(OUT, ">>/home/jbrunelle/public_html/wsdl/damage_revist/dataFiles/CSSdistro.csv");
                print OUT "$thePng, [$low,$mid,$high]\n";
               	close(OUT);
